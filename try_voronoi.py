...
#             cv.create_line(p2x,p2y,edge_inter3[-1][0],edge_inter3[-1][1],fill="red")
#             continue_button.wait_variable(var) #wait
#             break
...
def delete_nolink_line(lineset):
    ...
    for i in range(0,len(lineset),2):
        line_compare=[]
        line_compare+=lineset
        del line_compare[i]#刪除第i個點
        del line_compare[i]#刪除第i個點
        for j in range(len(line_compare)):
            if lineset[i][0]==line_compare[j][0] and lineset[i][1]==line_compare[j][1]:
                flag=1
                break                
        if flag==1:
           ans+=[lineset[i],lineset[i+1]]
           flag=0
           continue
        for j in range(len(line_compare)):
            if lineset[i+1][0]==line_compare[j][0] and lineset[i+1][1]==line_compare[j][1]:
                flag=1
                break
        if flag==1:
           ans+=[lineset[i],lineset[i+1]]
